...
- **Healthy 1:1** (`llama3_70b_32_direct`, `spine_count: 8`, no failures): symmetric all-to-all is capacity-matched *in aggregate* (each sender's NIC divides across its concurrent flows), but the matched budget holds only once senders stop restarting at line rate: measured run #110 showed the unwindowed fan-in-3 form never converging, because there is no sender congestion control in any arm (`CC_MODE 12`) and every flow restarts at line rate per bucket, so onset overload recurs faster than an unthrottled sender can drain it. The `direct2` window restores the designed behavior: congestion is purely episodic, the microburst incast at the destination downlink plus organic ECMP-collision hotspots. This is the well-run datacenter; it bounds how much benefit exists when the network is mostly clear. Run #117 measured this fabric as still a Knee regime under go-back-N (W 2.0-3.0 even with the burst disabled), because no sender congestion control means line-rate DP fan-in alone congests it.
- **Degraded 1:1** (`llama3_70b_64_direct*`, `spine_count: 8`, `failed_spine_count: 2`): two spines lost to failure or maintenance leave a 4:3 live ratio. The DP phase becomes network-bound: a 33% structural excess plus collision hotspots produce a heavy latency tail, and with no sender congestion control the fabric sits at the congestion knee, the regime where shedding a bounded fraction of gradient traffic buys super-linear relief. The knee is bracketed by measurement: sustained 2× overload collapses go-back-N recovery under trimming outright, and a healthy fabric leaves little to relieve.
- **Designed 2:1** (`llama3_70b_16`): the hard oversubscribed condition, kept because its dp=2 phase is small enough to grind through. `llama3_70b_64_sr2x` (below) already showed this ratio at W 0.02 under selective repair; a full oversubscription sweep under selective repair is the regime map's H1 (see [docs/agents/next-steps-after-run-117.md](../../docs/agents/next-steps-after-run-117.md)).
...
- [profiles/llama3_70b_64_sr2x.json](profiles/llama3_70b_64_sr2x.json): the selective-repair canary on the designed-2:1 fabric that collapsed go-back-N. `transport_recovery.selective_repair` makes the sender retransmit exactly the reported trimmed or missing byte ranges while the receiver accepts out-of-order payload; timeout recovery stays go-back-N as the silent-loss fallback. This pair completed in CI run #117: W dropped from 8.68-10.37 (go-back-N, same fabric family) to 0.02. Sustained oversubscription under selective repair is now a swept axis in the regime map (see [docs/agents/next-steps-after-run-117.md](../../docs/agents/next-steps-after-run-117.md)).
...
The trace samples one transformer-layer collective pattern across two gradient-accumulation microbatches: each sampled step emits 4 TP All-Reduces (one forward, one backward, per microbatch) of $4096\times8192\times2=67{,}108{,}864$ bytes per rank, then one local DP bucket. It is therefore a bounded production-shaped event window, not a literal replay of all 80 layers and all 256 gradient buckets. Across 16 ranks and the profile's 20 steps, those input payloads total 80 GiB of TP traffic plus about 20.4 GiB of sampled DP traffic; the retained $7\times128$ MiB incast adds 0.875 GiB once, at step 18. At the configured 4 KiB payload size this is approximately 26.5 million input payload packets before the native collective algorithm expands them into transport flows. Queue monitoring starts at simulated time zero and samples every 10 μs, preserving queue/PFC evidence without a monitor event per packet-scale interval.

The incast remains enabled deliberately: it is the paired study's exogenous, reproducible congestion stressor. `compare.py --require-congestion` rejects a run unless that traffic produces background-flow evidence, a nonzero queue peak, and (on this best-effort, PFC-off fabric) a nonzero trim or rejection count. The no-incast profile remains the separate negative control. This condition is not an exact Llama, Megatron, DeepSpeed, or PyTorch replay; it does not establish a 100-iteration warmup or a complete 140 GB gradient synchronization. A full framework trace is required for those claims.
